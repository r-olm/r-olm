...
impl EncryptionKeys {
    ...
    pub fn authenticate(&self, input: &[u8]) -> Vec<u8> {
        let mut mac = HmacSha256::new_varkey(&self.hmac_key)
            .expect("error while creating MAC instance from key");
        mac.input(input);
...
mod test {
    ...
    fn check_encryption() {
        ...
        let msg = b"input msg";
        let cipher_text = enc_keys
            .encrypt(msg)
            .expect("error while encrypting with OpenSSL");
