...
impl RootChainKeys {
    ...
    // Output = Ri, Ci,0
    pub fn advance_root_key(
        &self,
        previous_root_key: &[u8],
        previous_ratchet_key: &[u8; 32],
        current_ratchet_key: &[u8; 32],
    ) -> (Vec<u8>, Vec<u8>) {
        ...
        let ratchets_shared = diffie_hellman(previous_ratchet_key, current_ratchet_key);

        let info = "OLM_RATCHET";
        let length = 64;

        let hkdf_extract = Hkdf::<Sha256>::extract(previous_root_key, &ratchets_shared);
    ...
    pub fn advance_chain_key(&self, previous_chain_key: &[u8]) -> Vec<u8> {
        let mut mac_chain_key = HmacSha256::new_varkey(previous_chain_key)
            .expect("Error on HMACing when advaning chain key");
    ...
    pub fn create_message_key(&self, current_chain_key: &[u8]) -> Vec<u8> {
        let mut mac_chain_key = HmacSha256::new_varkey(current_chain_key)
            .expect("Error on HMACing when creating message key");
...
mod test {
    ...
    fn short_invalid_length_shared_secret() {
        let init_root_setup = RootChainKeys {
            shared_secret: vec![0x01; 20],
        };
